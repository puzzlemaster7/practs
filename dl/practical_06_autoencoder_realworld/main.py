...
def run() -> None:
    ...
    autoencoder.fit(x_train, x_train, epochs=35, batch_size=64, verbose=0, validation_split=0.1)

    recon = autoencoder.predict(x_test[:10], verbose=0)
    ...
    for i in range(10):
        ...
        plt.axis("off")
    plt.suptitle("Autoencoder Reconstructions (sklearn digits)")
    plt.tight_layout()
    out_path = out_dir / "reconstructions.png"
    plt.savefig(out_path, dpi=160)
    plt.close()

    mse = float(np.mean((x_test[:10] - recon) ** 2))
